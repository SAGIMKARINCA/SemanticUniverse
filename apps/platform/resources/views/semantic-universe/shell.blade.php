...
(() => {
    ...
    const applyFrameMode = (mode) => { document.body.classList.remove('su-frame-fixed', 'su-frame-scroll', 'su-frame-hidden'); document.body.classList.add(`su-frame-${mode}`); localStorage.setItem('semantic-universe-frame-mode', mode); frameSelects.forEach((select) => select.value = mode); };
    ...
    applyFrameMode(localStorage.getItem('semantic-universe-frame-mode') || 'fixed');
